...
export function normalizeRepairStatus(status) {
  ...
  return REPAIR_STATUS_TO_LEGACY[normalized] || normalized.toLowerCase()
}

export function repairStatusToErp(status) {
...
export function normalizeRepairRequest(item) {
  if (!item) return null
  ...
  return {
    ...
    created_at: item.reportedAt || item.createdAt,
