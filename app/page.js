...
export default function Home() {
  ...
  return (
    <main style={{ minHeight: '100vh' }}>
      <div style={{ background: 'var(--card)', padding: '20px 24px', position: 'sticky', top: 0, zIndex: 100, borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 480, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 12 }}>
          {view !== 'main' && (
            <button onClick={goBack} style={{ width: 36, height: 36, borderRadius: 12, border: 'none', background: 'var(--bg)', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              &larr;
            </button>
          )}
        ...
        {view === 'main' && (
          <>
            <div style={{ padding: '32px 0 8px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 14, color: 'var(--text-sub)', marginTop: 6 }}>문제 상황을 선택하거나 검색해주세요</p>
            </div>
            ...
            <div style={{ padding: '20px 0 4px', position: 'relative' }}>
              <input
                ...
                style={{ width: '100%', padding: '14px 16px', border: 'none', borderRadius: 16, fontSize: 15, background: 'var(--card)', boxShadow: 'var(--shadow)', outline: 'none', color: 'var(--text)' }}
              />
            </div>
            ...
            {!searchQuery.trim() && (
              <>
                ...
                <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-sub)', padding: '20px 0 10px' }}>어떤 문제인가요?</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                  {GUIDE_DATA.categories.map(cat => (
                    <div key={cat.id} onClick={() => showList(cat)} style={{ background: 'var(--card)', borderRadius: 'var(--radius)', padding: '24px 18px', cursor: 'pointer', boxShadow: 'var(--shadow)', textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--text-sub)' }}>{cat.desc}</div>
                      <div style={{ display: 'inline-block', marginTop: 8, padding: '3px 10px', background: 'var(--bg)', borderRadius: 8, fontSize: 11, fontWeight: 600, color: 'var(--text-sub)' }}>
              ...
              <div style={{ paddingBottom: 20 }}>
                {searchResults.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '60px 20px' }}>
                    <p style={{ color: 'var(--text-sub)', fontSize: 15 }}>&quot;{searchQuery}&quot; 검색 결과가 없습니다</p>
                  </div>
                ) : (
                  <>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-sub)', padding: '20px 0 10px' }}>{searchResults.length}개 결과</div>
        ...
        {view === 'detail' && selectedGuide && selectedCat && (
          <div>
            <div style={{ padding: '28px 0 20px' }}>
              ...
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: '#fff5eb', borderRadius: 12, fontSize: 13, color: 'var(--orange)', fontWeight: 600 }}>
                {selectedGuide.symptom}
              </div>
              {selectedGuide.shortcut && (
                <div style={{ marginTop: 14, padding: '12px 16px', background: 'var(--card)', borderRadius: 14, boxShadow: 'var(--shadow)', fontSize: 16, fontWeight: 800, textAlign: 'center', letterSpacing: 2 }}>
                  {selectedGuide.shortcut}
                </div>
              )}
            </div>
              ...
              {selectedGuide.steps.map((step, i) => (
                <div key={i} style={{ position: 'relative', paddingLeft: 48, paddingBottom: 20 }}>
                  {i < selectedGuide.steps.length - 1 && (
                    <div style={{ position: 'absolute', left: 19, top: 40, bottom: 0, width: 2, background: 'var(--border)' }} />
                  )}
                  ...
                  <div style={{ background: 'var(--card)', borderRadius: 'var(--radius)', overflow: 'hidden', boxShadow: 'var(--shadow)' }}>
                    <div style={{ padding: 18 }}>
                      <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 6 }}>{step.action}</div>
                      <div style={{ fontSize: 13, color: 'var(--text-sub)', lineHeight: 1.65, whiteSpace: 'pre-line' }}>{step.detail}</div>
                    </div>
                    {step.image && (
                      <img src={step.image} alt={step.action} style={{ width: '100%', maxHeight: 360, objectFit: 'contain', background: '#fafbfc', borderTop: '1px solid var(--border)', display: 'block' }} />
                    )}
                  </div>
                </div>
              ))}
      ...
      <div style={{ textAlign: 'center', padding: '32px 20px' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '10px 18px', background: '#fff0f0', borderRadius: 12, fontSize: 13, fontWeight: 700, color: 'var(--red)', marginBottom: 12 }}>
          해결이 안 되면 본사 경영지원팀에 연락
        </div>
        <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>하카코리아 경영지원팀 &middot; {GUIDE_DATA.lastUpdate}</div>
      </div>
    </main>
  )
}
...
function GuideItem({ cat, guide, onClick }) {
  return (
    <div onClick={onClick} style={{ background: 'var(--card)', borderRadius: 'var(--radius)', padding: 20, marginBottom: 10, boxShadow: 'var(--shadow)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 16 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-sub)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{guide.symptom}</div>
        {guide.shortcut && (
          <div style={{ display: 'inline-block', marginTop: 6, padding: '3px 8px', background: 'var(--bg)', borderRadius: 6, fontSize: 12, fontWeight: 700 }}>
            {guide.shortcut}
          </div>
        )}
      </div>
      <span style={{ flexShrink: 0, color: 'var(--text-muted)', fontSize: 16 }}>&rsaquo;</span>
    </div>
  )
}
