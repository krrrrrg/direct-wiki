...
const StoreChartLazy = dynamic(() => import('../pos-dashboard/StoreChart'), { ssr: false })
...
export default function MonthlyReportPage() {
  ...
  useEffect(() => {
    async function fetchStores() {
      const { data } = await supabase.from('monthly_sales').select('store_name')
      if (data) {
  ...
  const downloadExcel = () => {
    ...
    XLSX.writeFile(wb, `월별매출_${startMonth}_${endMonth}.xlsx`)
  }
                ...
                <div className="flex items-center flex-1 h-11 rounded-xl border border-border/40 bg-background">
                  <button
                    onClick={() => {
                      ...
                      setStartMonth(m === 1 ? `${y-1}-12` : `${y}-${String(m-1).padStart(2, '0')}`)
                  ...
                  <button
                    onClick={() => {
                      const [y, m] = startMonth.split('-').map(Number)
                      setStartMonth(m === 12 ? `${y+1}-01` : `${y}-${String(m+1).padStart(2, '0')}`)
                      setActivePreset(null)
                    }}
                    className="px-3 h-full text-muted-foreground hover:text-primary transition-colors"
                  >
                ...
                <div className="flex items-center flex-1 h-11 rounded-xl border border-border/40 bg-background">
                  <button
                    onClick={() => {
                      ...
                      setEndMonth(m === 1 ? `${y-1}-12` : `${y}-${String(m-1).padStart(2, '0')}`)
                      setActivePreset(null)
                    }}
                    className="px-3 h-full text-muted-foreground hover:text-primary transition-colors"
                  >
                  ...
                  <button
                    onClick={() => {
                      const [y, m] = endMonth.split('-').map(Number)
                      setEndMonth(m === 12 ? `${y+1}-01` : `${y}-${String(m+1).padStart(2, '0')}`)
                      setActivePreset(null)
                    }}
                    className="px-3 h-full text-muted-foreground hover:text-primary transition-colors"
                  >
        ...
        {searched && !loading && (
          <>
            <div className="mb-4 space-y-2">
              ...
              <div className="flex items-center justify-between">
                <p className="text-[11px] text-muted-foreground">조회 기간: {startMonth} ~ {endMonth}</p>
                {salesData.length > 0 && (
                  ...
                  {salesData.map(d => (
                    <div key={d.store_name}>
                      <div
                        ...
                        className={`px-4 py-3 flex items-center hover:bg-secondary/30 transition-colors cursor-pointer min-w-[680px] ${expandedStore === d.store_name ? 'bg-primary/5' : ''}`}
                      >
                        ...
                        <div className="w-[90px] shrink-0 text-right"><p className="text-[12px] text-foreground">{fmt(d.total_card)}</p></div>
                        <div className="w-[90px] shrink-0 text-right"><p className="text-[12px] text-foreground">{fmt(d.total_cash_no_receipt)}</p></div>
                        <div className="w-[90px] shrink-0 text-right"><p className="text-[12px] text-foreground">{fmt(d.total_cash_receipt)}</p></div>
                        <div className="w-[90px] shrink-0 text-right"><p className="text-[12px] text-foreground">{fmt(d.total_transfer)}</p></div>
                        <div className="w-[90px] shrink-0 text-right"><p className="text-[12px] text-foreground">{fmt(d.total_expense)}</p></div>
                      </div>
                      {expandedStore === d.store_name && (
                        <StoreChartLazy storeName={d.store_name} tab="monthly" chartData={getStoreChartData(d.store_name)} />
                      )}
                    </div>
                  ))}
                </div>
