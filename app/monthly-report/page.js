...
export default function MonthlyReportPage() {
  ...
  useEffect(() => {
    async function fetchStores() {
      ...
      if (data) {
        ...
        const list = Array.from(unique).map(name => ({ store_name: name })).sort((a, b) => a.store_name.localeCompare(b.store_name, 'ko'))
        setStoreList(list)
      }
    }
    fetchStores()
    ...
    if (allData.length > 0) {
      ...
      allData.forEach(d => {
        if (!grouped.has(d.store_name)) {
          grouped.set(d.store_name, { store_name: d.store_name, total_amount: 0, total_card: 0, total_cash_no_receipt: 0, total_cash_receipt: 0, total_transfer: 0 })
        }
  ...
  const downloadExcel = () => {
    const rows = salesData.map(d => ({
      '매장명': d.store_name, '카드': d.total_card, '현금(무)': d.total_cash_no_receipt,
      '현금영수증': d.total_cash_receipt, '이체': d.total_transfer, '합계': d.total_amount,
    }))
          ...
          <CardContent className="p-5 space-y-4">
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {presets.map(p => (
                  <button key={p.label} onClick={() => { setStartMonth(p.start); setEndMonth(p.end); setActivePreset(p.label) }}
